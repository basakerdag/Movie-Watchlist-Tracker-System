...
const MovieService = {
    ...
    validateAndUpdateMovie: (id, movieData, callback) => {
        ...
        if (!id) return callback(new Error('Invalid movie ID for update.'));
        if (!title || title.trim() === '') return callback(new Error('Movie title cannot be empty.'));
        if (!status || (status !== 'To Watch' && status !== 'Watched')) return callback(new Error('Invalid watchlist status.'));
        if (release_year && (release_year < 1888 || release_year > 2026)) return callback(new Error('Invalid release year.'));
        
        // Re-enforce business rules on update
        if (status === 'To Watch' && rating) {
            return callback(new Error('You cannot rate a movie that you have not watched yet.'));
        }
        if (status === 'Watched' && (!rating || rating < 1 || rating > 5)) {
